...
describe("contracts", () => {
  ...
  it(".env.example documents all runtime configuration keys without real secrets", () => {
    ...
    expect(envExample).not.toContain("LEASE_SECONDS=");
  ...
  it("CI and deploy workflows run the sensitive content scanner", () => {
    ...
    expect(packageJson).toContain('"scan:production": "tsx scripts/scan-production-readiness.ts"');
    expect(ciWorkflow).toContain("pnpm scan:sensitive");
    expect(ciWorkflow).toContain("pnpm scan:production");
  ...
  it("Docker image is a shared non-root runtime for receiver and worker", () => {
    ...
    expect(terraform).toContain("invoker_iam_disabled = true");
    expect(terraform).not.toContain("member   = \"allUsers\"");
