...
- `emojis` の名前は一意．
- 有効な絵文字が6件以上存在する．
- fallbackの3件は `enabled: true` な候補である．
...
既定カタログは，確認，称賛，感謝，議論，調査，注意，予定，文書，修理，
緊急，ヘルプ，軽い笑いなど，チームメイトの自然な反応として使える
標準絵文字だけで構成する．これにより，利用者はSlackワークスペースへ
カスタム絵文字を事前登録せずに導入できる．
...
2. 利用不可の候補は飛ばす．
3. 3件に満たない場合，設定ファイルで有効な絵文字候補を記載順に追加する．
4. 重複を除外する．
